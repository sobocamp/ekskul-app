...
class PembinaUpdateRequest extends FormRequest
{
    /**
     ...
     * atau gate untuk membatasi siapa yang boleh memperbarui data pembina.
     *
    ...
    /**
     * Aturan validasi untuk memperbarui data pembina.
     *
     ...
     *      - harus unik di tabel users, tetapi mengabaikan email pembina yang sedang diedit.
     ...
     * - **role**:
     *      - wajib diisi.
     *      - harus bernilai 'pembina'.
     *
     * Request ini digunakan pada update data pembina dalam `PembinaController@update`.
     *
    ...
    public function rules(): array
    {
        return [
            ...
            // Email harus unik tetapi abaikan id pembina yang sedang diedit
            'email' => [
                'required',
                'string',
                'email',
                'max:255',
                Rule::unique('users', 'email')->ignore($this->route('pembina'))
            ],

            // Password opsional, tetapi jika diisi harus minimal 8 karakter
            'password' => ['nullable', 'string', 'min:8'],

            // Role harus tetap 'pembina'
